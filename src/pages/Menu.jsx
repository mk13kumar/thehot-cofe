...
const Menu = ({ whatsappNumber, menuCategories = [], onOrder = () => {} }) => {
  ...
  const buildCartText = () => {
    ...
    const lines = cartItems.map((entry) => {
      const priceLabel = entry.price ? formatAmount(entry.price) : formatPrices(entry.prices)
      return `- ${entry.quantity} × ${entry.name} (${priceLabel})`
    })

    return `Hello the hot & cold cafe, please take my order:\n${lines.join('\n')}`
  }
